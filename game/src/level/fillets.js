...
export function makeFillet(p, size, spec, heightAt, paint = 'grass') {
  ...
  const pts = [];
  if (spec.r) {
    const r = spec.r, n = Math.max(10, Math.round((2 * Math.PI * r) / 0.75));
    ...
    const side = (x0, z0, x1, z1, nx, nz) => { const L = Math.hypot(x1 - x0, z1 - z0), n = Math.max(1, Math.round(L / 0.75)); for (let i = 0; i < n; i++) { const t = i / n; pts.push([x0 + (x1 - x0) * t, z0 + (z1 - z0) * t, nx, nz]); } };
  ...
  // round 2 (integrator): outline points every 0.75 m (was 0.5): the ring is a soft blend, 67k triangles in view at the hairpin exit
