 ...
 *
 * The chase camera of docs/TRACK-PLAN.md section 9:
 *   6.8 m behind the kart along its smoothed heading (7.6 m at vmax), 2.4 m above the ROAD
 *   under the kart (a hop does not lift the camera), looking at a point 5 m ahead of the kart
 *   1.0 m up; vertical fov 60 on landscape and 78 on portrait, plus 5 degrees at the speed cap
 *   and a further 8 on boost; yaw follows the heading through a critically damped spring with
 *   a 0.18 s time constant and lags up to 12 degrees toward the outside during a drift; roll
 *   is 60% of the road bank; a sphere cast from the kart to the camera pulls it in when a wall
 *   is between; the camera never pitches below the horizon minus 20 degrees.
 *
...
export const CAMERA = {
  ...
  fovLandscape: 60, fovPortrait: 78, fovSpeed: 5, fovBoost: 8,
  yawTau: 0.18, driftLagDeg: 12, bankRoll: 0.6, pitchFloorDeg: -20,
  castRadius: 0.4, heightTau: 0.12, fovTau: 0.25,
  ...
  _place(body, dt, hard) {
    ...
    if (w && typeof w.sphereCast === 'function') {
      ...
      if (hit && hit.hit) {
        ...
        const d = clamp((typeof hit.dist === 'number' ? hit.dist : full) - 0.2, 1.2, full);
