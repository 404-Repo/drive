// house_arcade c0: primitive assembly. Square stone piers with base bands carry ring archivolts
...
export default function (THREE) {
  ...
  for (const x of [-4.32, 4.32]) { box(BW, 1.0, 1.3, 0.02, glass, x, 1.7, 0.01); box(BW, 1.2, 0.1, 0.06, frameM, x, 2.4, 0.03); box(BW, 1.2, 0.1, 0.06, frameM, x, 1.0, 0.03); for (const s of [-1, 1]) box(BW, 0.1, 1.4, 0.06, frameM, x + s * 0.55, 1.7, 0.03); }
  ...
  for (const x of [-2.6, 0, 2.6]) { box(BK, 1.0, 1.4, 0.02, glass, x, 1.9, 0.01); box(BK, 1.24, 0.1, 0.06, frameM, x, 2.65, 0.03); box(BK, 1.24, 0.1, 0.06, frameM, x, 1.15, 0.03); for (const s of [-1, 1]) box(BK, 0.1, 1.6, 0.06, frameM, x + s * 0.57, 1.9, 0.03); box(BK, 1.4, 0.1, 0.2, stone, x, 1.1, 0.08); }
  for (const s of [1, -1]) { const F = face(s * PI / 2, s * W / 2, 0); for (const x of [-2.9, 0, 2.9]) { win(F, x, 4.3, 1.1, 1.7); win(F, x, 7.5, 1.1, 1.7); } const bx = -s * (D / 2 - PW - WALK / 2); for (const x of [bx - s * 2.9, bx - s * 5.4]) { if (Math.abs(x) < D / 2 - 0.7) { box(F, 1.0, 1.4, 0.02, glass, x, 1.9, 0.01); box(F, 1.24, 0.1, 0.06, frameM, x, 2.65, 0.03); box(F, 1.24, 0.1, 0.06, frameM, x, 1.15, 0.03); for (const q of [-1, 1]) box(F, 0.1, 1.6, 0.06, frameM, x + q * 0.57, 1.9, 0.03); box(F, 1.4, 0.1, 0.2, stone, x, 1.1, 0.08); } } }
  ...
  const yR = GF + 0.22, n = Math.round((W + 0.2) / 0.2);
  ...
  for (const [A, B, TL, TR] of faces) {
    ...
    for (let s = -wB / 2 + 0.2; s < wB / 2 - 0.1; s += 0.4) { let t = 1; for (let k = 1; k <= 20; k++) { const tt = k / 20, half = (wB + (wT - wB) * tt) / 2; if (Math.abs(s) > half) { t = (k - 1) / 20; break; } } const len = Math.max(0.3, t * L - 0.1); add(F, new THREE.CylinderGeometry(0.075, 0.075, len, 6, 1, true), roof, s, len / 2, 0.075); }
  ...
  for (const x of [-3.2, 0, 3.2]) {
    ...
    box(Dm, 1.1, 1.0, 0.9, wall, 0, 0.1, 0.55); box(Dm, 0.5, 0.5, 0.02, glass, 0, 0.3, 1.01); box(Dm, 0.66, 0.66, 0.06, frameE, 0, 0.3, 1.0); box(Dm, 0.5, 0.5, 0.06, glass, 0, 0.3, 1.02);
